...
A production-ready **production-ready flask application for call recording via telnyx voice api** built with Python, Flask, and Migration, Number Porting, Voice.
...
| `POST` | `/calls/<call_control_id>/recording/start` | Start |
| `POST` | `/calls/<call_control_id>/recording/stop` | Stop |
...
- **`initiate_call_with_recording()`** — initiate call with recording
- **`start_recording()`** — start recording
- **`stop_recording()`** — stop recording
- **`hangup_call()`** — hangup call
- **`initiate_call_endpoint()`** — initiate call endpoint
- **`start_recording_endpoint()`** — start recording endpoint
- **`stop_recording_endpoint()`** — stop recording endpoint
