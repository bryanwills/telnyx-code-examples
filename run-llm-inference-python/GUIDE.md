...
A production-ready **run llm inference on telnyx — openai-compatible chat completions api** built with Python, Flask, and AI Inference.
...
The entire app is in `app.py` (115 lines). Here's how it's structured:

### Endpoints

| Method | Path | Purpose |
|--------|------|---------|
| `POST` | `/inference/chat` | Chat |
| `POST` | `/inference/ask` | Ask |
| `GET` | `/health` | Health check |

### Key Functions

- **`chat_completion()`** — chat completion
- **`simple_ask()`** — simple ask
- **`chat_endpoint()`** — chat endpoint
- **`ask_endpoint()`** — ask endpoint
- **`health()`** — health
