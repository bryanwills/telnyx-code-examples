...
A production-ready **ai price quote phone agent** built with Python, Flask, and AI Inference.
...
The entire app is in `app.py` (77 lines). Here's how it's structured:
...
curl -X GET http://localhost:5000/quotes \
...
```bash
docker build -t ai-price-quote-phone-agent-python .
docker run --env-file .env -p 5000:5000 ai-price-quote-phone-agent-python
```
