...
A production-ready **cnam caller id lookup enrichment** built with Python, Flask, and Number Lookup.
...
- **`enrich_inbound()`** — enrich inbound
- **`list_enrichments()`** — list enrichments
- **`health()`** — health
...
curl -X GET http://localhost:5000/lookup/<number> \
