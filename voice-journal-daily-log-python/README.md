...
```bash
docker build -t voice-journal-daily-log .
docker run --env-file .env -p 5000:5000 voice-journal-daily-log
```
...
| `JOURNAL_NUMBER` | Phone number in E.164 format | Yes |
...
| `GET` | `/journal/insights` | `GET` /journal/insights |
