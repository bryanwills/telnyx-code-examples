...
```bash
docker build -t ai-standup-facilitator-phone .
docker run --env-file .env -p 5000:5000 ai-standup-facilitator-phone
```
...
| `STANDUP_NUMBER` | Phone number in E.164 format | Yes |
| `SLACK_WEBHOOK_URL` | Slack incoming webhook URL for slack webhook url notifications | No |
...
| `GET` | `/standups/summary` | `GET` /standups/summary |
