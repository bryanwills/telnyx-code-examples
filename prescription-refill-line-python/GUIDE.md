...
A production-ready **prescription refill line** built with Python, Flask, and Voice, AI Inference.
...
cd telnyx-code-examples/prescription-refill-line-python
cp .env.example .env
...
| `GET` | `/refills` | Refills |
| `POST` | `/refills/<int:idx>/approve` | Approve |
| `POST` | `/refills/<int:idx>/deny` | Deny |
...
- **`list_refills()`** — list refills
- **`approve_refill()`** — approve refill
- **`deny_refill()`** — deny refill
- **`health()`** — health
...
curl -X GET http://localhost:5000/refills \
