...
| `PHARMACY_SLACK_WEBHOOK` | Slack incoming webhook URL for pharmacy notifications | No |
...
| `POST` | `/refills/<int:idx>/approve` | Approve a pending item (triggers customer notification) |
| `POST` | `/refills/<int:idx>/deny` | Reject/deny a pending item (triggers customer notification) |
