...
A production-ready **sms emergency check-in** built with Python, Flask, and SMS/MMS.
...
The entire app is in `app.py` (83 lines). Here's how it's structured:
...
| `POST` | `/check-in/send` | Send |
| `POST` | `/webhooks/messaging` | Telnyx webhook handler |
| `POST` | `/check-in/escalate` | Escalate |
| `GET` | `/status` | Status |
...
- **`send_sms()`** — send sms
- **`add_monitored()`** — add monitored
- **`send_check_ins()`** — send check ins
- **`handle_reply()`** — handle reply
- **`escalate_missed()`** — escalate missed
