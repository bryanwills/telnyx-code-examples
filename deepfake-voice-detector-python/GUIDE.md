...
A production-ready **deepfake voice detector** built with Python, Flask, and Voice, AI Inference, Media Streaming.
...
| **Products** | Voice, AI Inference, Media Streaming |
| **Channels** | voice |
...
The entire app is in `app.py` (345 lines). Here's how it's structured:
...
| `POST` | `/calls/<call_id>/analyze` | Analyze |
| `GET` | `/calls` | Calls |
...
- **`analyze_with_inference()`** — analyze with inference
- **`send_alert()`** — send alert
- **`telnyx_action()`** — telnyx action
- **`handle_voice()`** — handle voice
- **`handle_media()`** — handle media
- **`force_analyze()`** — force analyze
