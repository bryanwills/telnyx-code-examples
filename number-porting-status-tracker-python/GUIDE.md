...
A production-ready **number porting status tracker** built with Python, Flask, and SMS/MMS.
...
|--------|------|---------|
| `GET` | `/ports/list` | List |
| `POST` | `/ports/create` | Create |
| `POST` | `/webhooks/porting` | Telnyx webhook handler |
| `GET` | `/ports/<order_id>` | <Order Id> |
...
- **`handle_porting_webhook()`** — handle porting webhook
- **`get_port()`** — get port
- **`health()`** — health
...
curl -X GET http://localhost:5000/ports/list \
