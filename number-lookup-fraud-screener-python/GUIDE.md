...
A production-ready **number lookup fraud screener** built with Python, Flask, and Number Lookup.
...
The entire app is in `app.py` (105 lines). Here's how it's structured:

### Endpoints

| Method | Path | Purpose |
|--------|------|---------|
| `GET` | `/screen/<number>` | <Number> |
| `POST` | `/webhooks/voice` | Telnyx webhook handler |
| `POST` | `/blocklist` | Blocklist |
| `GET` | `/blocklist` | Blocklist |
| `GET` | `/screening-log` | Screening Log |
...
- **`screen_inbound_call()`** — screen inbound call
- **`add_to_blocklist()`** — add to blocklist
- **`list_blocklist()`** — list blocklist
- **`get_log()`** — get log
- **`health()`** — health
...
curl -X GET http://localhost:5000/screen/<number> \
