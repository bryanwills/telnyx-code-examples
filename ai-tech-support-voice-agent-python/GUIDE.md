...
A production-ready **ai tech support voice agent — it helpdesk voice agent with knowledge base** built with Python, Flask, and AI Inference.
...
cd telnyx-code-examples/ai-tech-support-voice-agent-python
cp .env.example .env
...
The entire app is in `app.py` (67 lines). Here's how it's structured:
...
```bash
docker build -t ai-tech-support-voice-agent-python .
docker run --env-file .env -p 5000:5000 ai-tech-support-voice-agent-python
```
