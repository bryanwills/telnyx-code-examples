...
A production-ready **build a complete voice ai agent with telnyx** built with Python, Flask, and AI Inference.
...
The entire app is in `app.py` (190 lines). Here's how it's structured:
...
- **`call_telnyx_inference()`** — call telnyx inference
...
```bash
docker build -t build-voice-ai-agent-python .
docker run --env-file .env -p 5000:5000 build-voice-ai-agent-python
```
