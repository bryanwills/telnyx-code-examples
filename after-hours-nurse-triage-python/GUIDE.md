...
A production-ready **after-hours nurse triage** built with Python, Flask, and Voice, AI Inference.

Integrates with PagerDuty, Slack for extended functionality.
...
The entire app is in `app.py` (160 lines). Here's how it's structured:
...
| `GET` | `/triage/queue` | Queue |
| `POST` | `/triage/<int:idx>/override` | Override |
| `GET` | `/health` | Health check |

### Key Functions

- **`ai_triage()`** — ai triage
- **`page_oncall()`** — page oncall
- **`notify_nurse()`** — notify nurse
- **`send_sms()`** — send sms
...
curl -X GET http://localhost:5000/triage/queue \
...
- Extend PagerDuty, Slack integration with richer workflows
