...
A production-ready **insurance claims intake** built with Python, Flask, and Voice, AI Inference.
...
cd telnyx-code-examples/insurance-claims-intake-python
cp .env.example .env
...
The entire app is in `app.py` (111 lines). Here's how it's structured:
...
| `GET` | `/claims` | Claims |
| `POST` | `/claims/<claim_id>/assign` | Assign |
...
```bash
docker build -t insurance-claims-intake-python .
docker run --env-file .env -p 5000:5000 insurance-claims-intake-python
```
