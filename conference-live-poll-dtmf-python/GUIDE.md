# Build a Conference Live Poll via DTMF
...
Everything lives in `app.py` (108 lines). Here's what each piece does.
...
**`start_poll()`** — Kicks off the main workflow. Validates the request, creates the record, and initiates the Telnyx API calls.
...
```bash
curl http://localhost:5000/conference/<cid>/results | python3 -m json.tool
```
...
- [Source code and reference](https://raw.githubusercontent.com/team-telnyx/telnyx-code-examples/main/conference-live-poll-dtmf-python/README.md)
