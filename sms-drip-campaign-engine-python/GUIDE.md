...
A production-ready **sms drip campaign engine** built with Python, Flask, and SMS/MMS, AI Inference.
...
| Method | Path | Purpose |
|--------|------|---------|
| `POST` | `/drip/create` | Create |
| `POST` | `/drip/<did>/subscribe` | Subscribe |
| `POST` | `/drip/advance` | Advance |
...
- **`send_sms()`** — send sms
- **`create_drip()`** — create drip
- **`subscribe()`** — subscribe
- **`advance_all()`** — advance all
