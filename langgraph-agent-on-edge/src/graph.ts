...
import { lookupOrder, smalltalkFallback, extractOrderId } from "./tools.js";
...
const SYSTEM_PROMPT =
  "You are a concise SMS support agent for Telnyx Logistics. " +
  "Answer in one or two sentences. If the user asks about an order, the action node will look it up.";

const INTENT_SYSTEM_PROMPT =
  "Classify the user's message intent. Reply with exactly one word: " +
  "'order' if they ask about an order, delivery, or shipment. " +
  "'smalltalk' for greetings, thanks, or general chat. " +
  "Reply with only the word, no punctuation.";

const RESPONSE_SYSTEM_PROMPT =
  "You are replying to a customer SMS. Keep it to one or two sentences. " +
  "If order details are provided, summarize them naturally.";
...
export function buildGraph(env: Env, model: string) {
  ...
  async function intentNode(state: GraphStateType): Promise<Partial<GraphStateType>> {
    ...
    const intent: Intent = trimmed.includes("order") ? "order" : trimmed.includes("smalltalk") ? "smalltalk" : "unknown";
  ...
  async function actionNode(state: GraphStateType): Promise<Partial<GraphStateType>> {
    ...
    if (orderId) {
      ...
      return {
        actionResult: `Order ${order.orderId}: status=${order.status}, ETA=${order.eta}, carrier=${order.carrier}.`,
      };
    }
    return { actionResult: smalltalkFallback() };
  }

  async function responseNode(state: GraphStateType): Promise<Partial<GraphStateType>> {
    const systemContent = state.actionResult
      ? `${RESPONSE_SYSTEM_PROMPT}\n\nOrder context: ${state.actionResult}`
      : RESPONSE_SYSTEM_PROMPT;
    const result = await llm.invoke([
  ...
  const graph = new StateGraph(GraphState)
    ...
    .addConditionalEdges("intent", (state: GraphStateType) => {
      return state.intentLabel === "order" ? "action" : "response";
    })
