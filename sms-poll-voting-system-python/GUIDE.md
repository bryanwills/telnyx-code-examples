...
A production-ready **sms poll voting system — text-to-vote polling with real-time results** built with Python, Flask, and SMS/MMS.
...
|--------|------|---------|
| `POST` | `/polls` | Polls |
| `POST` | `/polls/<pid>/broadcast` | Broadcast |
...
- **`broadcast_poll()`** — broadcast poll
- **`handle_vote()`** — handle vote
- **`results()`** — results
- **`health()`** — health
