# Messaging Campaign Ab Test Optimizer
...
```bash
docker build -t messaging-campaign-ab-test-optimizer .
docker run --env-file .env -p 5000:5000 messaging-campaign-ab-test-optimizer
```
...
| `CAMPAIGN_NUMBER` | Phone number in E.164 format | Yes |
...
| `GET` | `/campaigns/<cid>/analyze` | `GET` /campaigns/<cid>/analyze |
...
```bash
curl http://localhost:5000/campaigns/<cid>/analyze
```
